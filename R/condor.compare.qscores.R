...
#' @param scale.log TRUE/FALSE - If TRUE, plot log10 core scores
...
#' @export
#' 
condor.compare.qscores <- function(x, y, by=c("column","row","both"), seed=8518,
                                   label.x="Condition x", label.y="Condition y",
                                   red.name="red", blue.name="blue", scale.log=TRUE,
                                   nsamp=1e4, node.type=c("both","red", "blue"), notch=TRUE) {
  by <- match.arg(by)
  ...
  p <- ggplot(d, aes(x=variable, y=value, color=is.stable, fill=is.stable, label=p), environment=environment()) +
  ...
  if (scale.log) {
    p <- p + scale_y_log10(breaks=10^seq(-20,0,1))
  }
  p
}