...
```{r}
condor.plot.heatmap(condor.object, blue.name="Plants", red.name="Pollinators", add.color=TRUE)
```
...
```{r}
condor.plot.heatmap(condor.object.noisy, blue.name="Plants", red.name="Pollinators", add.color=TRUE)
```

We examine the overlaps between community assignments for both plants and pollinators:
...
7 communities instead of 8 are identified in the noise-added data-set. We observe a minority of plants and pollinators that are no longer clustered together.
